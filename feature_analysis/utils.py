...
def make_jutted_tensor(orig_image):
    images_jutted_color = []
    for i in np.linspace(0.2, 1.0, 15):
        images_jutted_color.append(jit_of_color(orig_image * 0.5 + 0.5, brightness=i, visualise=False))
    images_jutted_color.append(orig_image)
    for i in np.linspace(1.5, 3.0, 15):
        images_jutted_color.append(jit_of_color(orig_image * 0.5 + 0.5, brightness=i, visualise=False))
...
def plot_augmented_features(
    ...
    if mid_idx is None:
        mid_idx = int(len(image_tensor) // 2)
    ...
    if ticklabels:
        ...
        original = ((resize(image_tensor[mid_idx]) + 1) * 127.5).numpy().swapaxes(0, 2).swapaxes(0, 1).astype(np.uint8)
